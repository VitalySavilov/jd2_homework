...
public class DaoImpl implements Dao {
    ...
    public Receiver getReceiver(int num) {
        ...
        try (final Connection connection = dataSource.getConnection()) {
            ...
            final ResultSet resultSet = statement.executeQuery("SELECT * " +
                    "FROM vitaly_savilov.receivers WHERE num=" + num);
    ...
    public Expense getExpense(int num) {
        ...
        try (final Connection connection = dataSource.getConnection()) {
            final Statement statement = connection.createStatement();
            final ResultSet resultSet = statement.executeQuery("SELECT * " +
                    "FROM vitaly_savilov.expenses WHERE num=" + num);
            while (resultSet.next()) {
    ...
    public int addReceiver(Receiver receiver) {
        int result = 0;
        try (final Connection connection = dataSource.getConnection()) {
            String receiversSQL = "INSERT INTO vitaly_savilov.receivers (name) " +
                    "SELECT * " +
    ...
    public int addExpense(Expense expense) {
        int result = 0;
        try (final Connection connection = dataSource.getConnection()) {
            String expensesSQL = "INSERT INTO vitaly_savilov.expenses (paydate, receiver, value) " +
                    "VALUES (?, ?, ?)";
