    ...
    public void readNames() {
        try (final Connection connection = dataSource.getConnection()) {
            ...
            String query = "select r.name, sum(e.value) as sum " +
                    "from vitaly_savilov.receivers as r " +
                    "join vitaly_savilov.expenses as e " +
                    "on e.receiver=r.num " +
                    "group by r.num";
    ...
    public void readSum() {
        try (final Connection connection = dataSource.getConnection()) {
            ...
            String query = "select paydate, sum(value) as sum " +
                    "from vitaly_savilov.expenses as e, " +
                    "(select max(value) as m " +
                    "from vitaly_savilov.expenses) as t " +
                    "where e.value=t.m " +
                    "group by paydate";
            ResultSet result = statement.executeQuery(query);
    ...
    public void readValue() {
        try (final Connection connection = dataSource.getConnection()) {
            ...
            String query = "select paydate, name, value " +
                    ...
                    "from vitaly_savilov.expenses " +
                    "group by p " +
                    "order by s desc " +
                    "limit 1) as t, " +
                    "vitaly_savilov.receivers as r " +
